...
private DietaData dt=new DietaData();
...
public void guardarMenu (MenuDiario menuDiario){
    String sql= "INSERT INTO menudiario(diaNro,estado,codDieta) VALUES(?,?,?)";
        ...
        ps.setInt(3, menuDiario.getDieta().getCodDieta());
        ps.executeUpdate();
    ...
    public MenuDiario buscarMenuPorId(int codMenu) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                menu.setEstado(true);
                menu.setDieta(dt.buscarDietaPorId(rs.getInt("codDieta")));
            } else {
    ...
    public ArrayList<MenuDiario> listarMenu(){
    
    ArrayList<MenuDiario> menu=new ArrayList<>();
